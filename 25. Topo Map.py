#Sanjidah Abdullah
#sabdull002@citymail.cuny.edu

import numpy as np
import matplotlib.pyplot as plt

blue = float(input("How blue is the ocean: "))
output = input("What is the output file: ")
print("Thank you for using my program!")
print("Your map is stored", output,".")

#Read in the data to an array, called elevations:
elevations = np.loadtxt('elevationsNYC.txt')

#Take the shape (dimensions) of the elevations
#  and add another dimension to hold the 3 color channels:
mapShape = elevations.shape + (3,)

#Create a blank image that's all zeros:
floodMap = np.zeros(mapShape)

for row in range(mapShape[0]):
    for col in range(mapShape[1]):
        if elevations[row,col] <= -10:
            #Below sea level
            floodMap[row,col,0] = 0.2     #Set the red channel to 20%
            floodMap[row,col,1] = 0.2     #Set the green channel to 20%
            floodMap[row,col,2] = 0.2     #Set the blue channel to 20%
        elif elevations[row,col] <= 0:
            #Below sea level but not so deep
           floodMap[row,col,2] = blue     #Set blue channel to user specified
        elif elevations[row,col] % 10==0:
            floodMap[row,col,0] = 0     #Set the red channel to 0%
            floodMap[row,col,1] = 0     #Set the green channel to 0%
            floodMap[row,col,2] = 0     #Set the blue channel to 0%
        else:
            floodMap[row,col,0] = 1.0   #Set the red channel to 100%
            floodMap[row,col,1] = 1.0   #Set the green channel to 100%
            floodMap[row,col,2] = 1.0   #Set the green channel to 100%

#Load the flood map image into matplotlib.pyplot:
plt.imshow(floodMap)

#Display the plot:
plt.show()

#Save the image:
plt.imsave(output, floodMap)



